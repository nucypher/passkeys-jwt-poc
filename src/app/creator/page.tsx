...
export default function CreatorPage() {
  ...
  const checkSession = async () => {
    try {
      const stored = localStorage.getItem("creatorSession");
      if (stored) {
      ...
      localStorage.removeItem("creatorSession");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleUserSetup = async (name: string) => {
    try {
      ...
      localStorage.setItem("creatorSession", JSON.stringify(sessionData));
      ...
      <div className="min-h-screen flex flex-col items-center justify-center p-8">
        <div className="max-w-md w-full text-center space-y-6">
          ...
          <button
            ...
            className="w-full rounded-full border border-solid border-transparent transition-colors flex items-center justify-center bg-blue-600 text-white gap-2 hover:bg-blue-700 font-medium text-base h-12 px-5"
          >
            Get Started
          </button>
          <Link
  ...
  if (!session && needsSetup) {
    return (
      <div className="min-h-screen flex items-center justify-center p-8">
        <UserSetup role="creator" onComplete={handleUserSetup} />
      </div>
    );
  }
